...
-- Drop the existing file_storage table
DROP TABLE IF EXISTS file_storage;

-- Create the updated file_storage table
CREATE TABLE file_storage (
                              id BIGINT NOT NULL AUTO_INCREMENT,
                              name VARCHAR(255) NOT NULL,            -- File name
                              type VARCHAR(100) NOT NULL,            -- File type (MIME type or custom type)
                              path VARCHAR(500) NOT NULL,            -- Path to the file on the file server
